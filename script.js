...
function typeWriter(element, text, speed = 100) {
    let i = 0;
    element.textContent = '';
    element.style.borderRight = '3px solid #f5576c';
    ...
    function type() {
        if (i < text.length) {
            ...
            setInterval(() => {
                element.style.borderRight = element.style.borderRight === 'none' ? '3px solid #f5576c' : 'none';
            }, 500);
        }
    }
    type();
...
window.addEventListener('load', () => {
    ...
    if (typewriterElement) {
        ...
        setTimeout(() => {
            typeWriter(typewriterElement, 'Endless Possibilities', 150);
        }, 1000);
    }
});
